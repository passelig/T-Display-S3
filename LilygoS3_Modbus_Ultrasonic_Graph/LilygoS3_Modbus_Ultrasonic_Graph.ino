...
int gy = 154;
int current = 0;
float pixelsPerCentimeter = gh / 240.0;
int yStartTemp = 0;
int personCount = 0;
...
void setup(void) {
  ...
  current = 150;  //dht.readTemperature();
...
void loop() {
  current = 200-personCount;  //dht.readTemperature();
  if (personCount>100){
    personCount = 0;
  }
  ...
  sprite.fillRoundRect(6, 5, 170, 35, 4, blue);
  sprite.drawString("Distance " + String(current), 14, 24, 4);
  sprite.fillRoundRect(180, 5, 130, 35, 4, blue);
  sprite.drawString("Count " + String(personCount), 190, 24, 4);
  sprite.setFreeFont();
  ...
  for (int i = 1; i < 7; i++) {
    ...
    sprite.drawString(String(i * 40), gx - 20, gy - (i * 17));
  }
  ...
  for (int i = 0; i < numOfValues - 1; i++) {
    sprite.drawLine(gx + (i * 2), gy - (measurmentPoints[i] - yStartTemp) * pixelsPerCentimeter, gx + ((i + 1) * 2), gy - (measurmentPoints[i + 1] - yStartTemp) * pixelsPerCentimeter, TFT_RED);
  }
